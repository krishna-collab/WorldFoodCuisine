...
WorldFoodCuisine is a delivery-only food brand: franchise kitchens with no
dining rooms, each cooking to order for the ZIP codes around it. The site
should feel like a good food magazine that orders like an app. Warm paper,
dark ink and one terracotta accent. A warm serif for headlines, a plain sans
for everything you read or tap. The food carries the pages; the ordering
controls stay quiet, large and predictable.
...
2. **Your ZIP picks the kitchen.** Each kitchen has its own menu for the day,
   prices, hours and delivery fee. The kitchen's name appears wherever a price
   does (dish page, menu, bag, checkout, confirmation), so it's always clear
   which location is cooking.
...
4. **Quiet, large controls.** One primary action per screen, in thumb reach on
   phones. Most tap targets are 40–56px (filter pills are 36px); nothing
   interactive is under 24px.
...
| `accent` = `clay` = `ring` | `#a63f1c`             | `#e58b62`             | Brand accent, ordering actions, links, focus ring  |
| `herb`                     | `#2d5a41`             | `#8fc7a2`             | Confirmations ("order placed", done steps)         |
| `warn-bg` / `warn-fg`      | `#fbefd0` / `#5e4100` | `#3a2c0f` / `#f4d07e` | Demo mode, drafts, "not yet"                       |
| `danger`                   | `#a3261b`             | `#f08a7e`             | Errors, dishes that can't be ordered               |
...
has a word next to the dots, and unavailable dishes say why.

## Type

- **Fraunces** (display serif, optical-size axis, weight 560) for headings,
  prices and totals. A metric-matched fallback keeps the
...
| `text-display-l`  | 32 → 56px             | Page titles, dish names          |
...
| body              | 16–17px, 1.6 leading  | Reading text                     |

Headings wrap balanced, paragraphs wrap "pretty". Numbers that change or line
up (prices, counts, totals) use `.nums` (tabular figures). Local dish
...
- Pinned bars never hide what has focus: the page reserves space for the
  header (`scroll-padding-top`) and the menu's pinned search row on phones.
...
| Dish facts                  | `components/food/dish-facts.tsx`                                                   | Heat (dots and a word), diet, the fat it's cooked in                                                                                           |
...
| Order panel                 | `routes/dish.$id.tsx`, `components/ordering/order-panel.tsx`                       | Kitchen, price, delivery time, options, allergens, quantity and Add to bag; asks for a ZIP code first                                          |
| Kitchen line, ZIP dialog    | `components/ordering/kitchen-line.tsx`, `location-dialog.tsx`                      | Which kitchen serves you and whether it's open; the ZIP check that finds it                                                                    |
...
| Locations                   | `routes/delivery.tsx`                                                              | Each kitchen's area, hours, fees, menu for the day and local prices                                                                            |
...
| Install, offline            | `components/pwa/`, `public/sw.js`                                                  | Install only when asked; saved dishes open offline; the bag is kept                                                                            |
...
- **Provenance.** Ingredients, allergens, story and flavor notes carry a
  `ContentRecord`: status (`draft`, `reviewed`, `verified`),
...
- **Demo commerce** always says so: "Demo kitchen West", "demo price",
  "Checkout (demo)", "Place demo order", "Nothing is sent or charged".
  Checkout has no card fields.
- **Delivery only.** Never suggest dining in, pickup counters or table service;
  the kitchens have none.
- **Voice.** Plain words, sentence case, no exclamation marks. Say what a
  thing does ("Add to bag", "Find your kitchen"), not what it is. When something isn't
...
- Live regions for things that change without a page load: the menu's result
  count and the ZIP check result.
